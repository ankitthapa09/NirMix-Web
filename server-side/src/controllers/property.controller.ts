...
class PropertyController {
   ...
   * Body: multipart/form-data with a `data` JSON field + new photos[] + optional floorPlan.
   */
  async updateProperty(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const validation = updatePropertySchema.safeParse(parsed);
      if (!validation.success) {
        const issue = validation.error.issues[0];
        const path = issue.path.join('.');
        throw new ApiError(
      ...
      res.status(HTTP_STATUS.OK).json(
        new ApiResponse(HTTP_STATUS.OK, property, 'Listing updated successfully')
      );
    } catch (error) {
      next(error);
    }
