...
import { pinIcon, colorForStatus } from "./markerIcons";
...
/** Browse-view map: a color-coded marker per pinned listing, linking to detail. */
export default function PropertiesMap({
  ...
  return (
    <MapFrame className={className} title="Properties map">
      {(expanded) => (
        <MapContainer center={NEPAL_CENTER} zoom={7} scrollWheelZoom={expanded} className="h-full w-full">
          <TileLayer
          ...
          {pinned.map((p) => (
            <Marker
              ...
              icon={pinIcon(colorForStatus(p.status))}
            >
              <Popup>
                <div className="w-44">
                  {p.photos[0] && (
