...
import { Check, Edit3, MapPin, CheckCircle, Info } from "lucide-react";
...
export function StepReview({ formData, onGoToStep, errors, onCheckboxChange }: StepReviewProps) {
  ...
  const formatLandArea = () => {
    const parts = [];
    if (formData.ropani) parts.push(`${formData.ropani} Ropani`);
    if (formData.aana) parts.push(`${formData.aana} Aana`);
    if (formData.paisa) parts.push(`${formData.paisa} Paisa`);
    if (formData.daam) parts.push(`${formData.daam} Daam`);
    
    if (parts.length > 0) return parts.join(" ");
    if (formData.landArea) return `${formData.landArea} Aana`;
    return "—";
  };
  ...
  return (
    <div className="space-y-6">
      {/* Summary Header Card */}
      <div className="flex flex-col sm:flex-row gap-5 p-5 rounded-2xl bg-white border border-[#E0D4C5] shadow-sm">
        <div className="w-full sm:w-44 aspect-video sm:aspect-square rounded-xl overflow-hidden bg-[#F5EFE6] shrink-0 border border-[#E0D4C5]">
          <img
            src={coverPhoto}
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
        {/* Column 1: Details */}
        <div className="p-5 rounded-2xl bg-white border border-[#E0D4C5] flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between pb-3 border-b border-[#E0D4C5]/50 mb-4">
              ...
              <button
                ...
                className="flex items-center gap-1 text-xs text-[#E0B33A] hover:text-[#C29624] font-bold cursor-pointer transition-colors"
              >
                Edit
              </button>
            </div>
            ...
            {propertyType === "Land" ? (
              <dl className="space-y-2.5 text-xs font-medium text-[#342417]/80">
                <div className="flex justify-between">
                  ...
                  <dd className="font-extrabold text-[#342417]">{formatLandArea()}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-[#342417]/50 font-bold">Zoning / land type:</dt>
                  <dd className="font-extrabold text-[#342417]">{formData.zoning || "—"}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-[#342417]/50 font-bold">Road access width:</dt>
              ...
              <dl className="space-y-2.5 text-xs font-medium text-[#342417]/80">
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="font-extrabold text-[#342417]">{carpetArea ? `${Number(carpetArea).toLocaleString()}` : "-"}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-[#342417]/50 font-bold">Built-up:</dt>
                  <dd className="font-extrabold text-[#342417]">{builtUpArea ? `${Number(builtUpArea).toLocaleString()}` : "-"}</dd>
                </div>
              </dl>
            )}

            {propertyType === "Land" && formData.amenities && formData.amenities.length > 0 && (
              <div className="mt-3 pt-3 border-t border-[#E0D4C5]/50">
                <p className="text-[11px] font-bold text-[#342417]/80 leading-relaxed">
      ...
      <div className="p-4 rounded-xl bg-[#FFC71E]/5 border border-[#E0B33A]/30 text-[#342417] text-xs">
          ...
          <div className="relative">
            ...
            <div
              className={`w-5 h-5 rounded-md border-2 transition-all flex items-center justify-center ${
                termsAccepted
                  ? "border-[#E0B33A] bg-[#E0B33A] text-[#342417]"
                  : "border-[#E0D4C5] bg-white hover:border-[#342417]/50"
              }`}
            >
